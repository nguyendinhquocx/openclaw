...
describe("memory forget source removal", () => {
  ...
  it("forgets a session's source inventory with bounded writes and preserves other sources", async () => {
    const { cfg } = fixture;
    ...
    const paths = Array.from(
      { length: 32 },
      (_, index) =>
        `sessions/main/target.jsonl.deleted.2026-08-25T10-00-00.${String(index).padStart(3, "0")}Z.zst`,
    );
